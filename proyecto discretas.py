...
def calcular_ruta():
    ...
    try:
        ...
        nx.draw(G, pos, with_labels=True, node_size=1800,
            node_color=colores_nodos, font_size=9   , font_weight="bold")   

        nx.draw_networkx_edges(G, pos, edgelist=ruta_aristas, width=5, edge_color="red")
        ...
        plt.legend(handles=leyenda,title= "referencias",loc="center left")

        plt.title(f"Ruta desde {origen} hasta {destino}", fontdict={'size': 14, 'weight': 'bold'})
        plt.axis("off")
        plt.tight_layout() #Fuerza a matplotlib a recalcular espacios y evitar colisiones
        fig = plt.gcf()  
        try:            
            fig.canvas.manager.window.state('zoomed')  # Maximiza la ventana del gráfico
        except:
            pass  # Si no se puede maximizar, simplemente se muestra en su tamaño actual
        plt.show()
...
combo_origen = ctk.CTkComboBox(ventana, values=lista_ciudades, width=250, state="readonly")
